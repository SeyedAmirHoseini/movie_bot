...
from .menu import show_admin_menu, VIDEOS_SUBMENU  # ایمپورت VIDEOS_SUBMENU از menu.py
core_session = {}
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not check_permission(uid, 'manage_videos'):
    ...
    # نمایش زیرمنوی ویدیوها
    if data == "videos_menu":
        await query.edit_message_text("🎥 بخش ویدیوها:", reply_markup=InlineKeyboardMarkup(VIDEOS_SUBMENU))
        return

    # شروع عملیات آپلود، ساخت لینک یا حذف
    ...
    return False  # اگر هیچ‌کدوم نبود، بره به هندلر بعدی

async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    uid = user.id
    if not check_permission(uid, 'manage_videos') or uid not in core_session:
        return False

    session = core_session[uid]
    msg = update.message
